...
export(NodePath) var show_achive_but
export(NodePath) var daily_but
export(NodePath) var weekly_but
export(NodePath) var monthly_but
export(NodePath) var full_but
export(NodePath) var next_but
export(NodePath) var score_back_but
export(NodePath) var achiv_back_but
...
func bb_animate_pressing_button(button_nr):
	...
	match button_nr:
		...
		2:
			button_texture=get_node(show_achive_but)
		3:
			button_texture=get_node(daily_but)
		4:
			button_texture=get_node(weekly_but)
		5:
			button_texture=get_node(monthly_but)
		6:
			button_texture=get_node(full_but)
		7:
			button_texture=get_node(next_but)
		8:
			button_texture=get_node(score_back_but)
		9:
			button_texture=get_node(achiv_back_but)
...
func _on_ChallangeTween_tween_all_completed():
	if anim_stoped:
		remove_all()
		button_texture.rect_scale = but_origin_scale
		button_texture.rect_position = but_origin_position
