...
export(NodePath) var keyboard_buts

export(NodePath) var del_but
export(NodePath) var spec_but
...
func bb_animate_pressing_button(button_nr):
	...
	match button_nr:
		100:
			button_texture=get_node(del_but)
		101:
			button_texture=get_node(spec_but)
		_:
			button_texture=get_node(keyboard_buts).get_child(button_nr)


	if used_button_nr!=button_nr:
		but_origin_position=button_texture.get("rect_position")
		but_origin_size=button_texture.get("rect_size")
		but_origin_scale=button_texture.get("rect_scale")
...
func _on_KeyboardTween_tween_all_completed():
	if anim_stoped:
		used_button_nr=-1
		remove_all()
		button_texture.rect_scale = but_origin_scale
