...
export(NodePath) var right_but
export(NodePath) var left_but
export(NodePath) var backlist_but
export(NodePath) var destroy_but
export(NodePath) var destroy_fullday_but
...
func bb_animate_pressing_button(button_nr):
	...
	match button_nr:
		...
		2:
			button_texture=get_node(right_but)
		3:
			button_texture=get_node(left_but)
		4:
			button_texture=get_node(backlist_but)
		5:
			button_texture=get_node(destroy_but)
		6:
			button_texture=get_node(destroy_fullday_but)
...
func _on_MemoryTween_tween_all_completed():
	if anim_stoped:
		remove_all()
		button_texture.rect_scale = but_origin_scale
		button_texture.rect_position = but_origin_position
