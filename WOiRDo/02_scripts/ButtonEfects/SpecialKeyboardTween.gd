...
export(NodePath) var lag_but
export(NodePath) var normal_but
...
func bb_animate_pressing_button(button_nr):
	...
	match button_nr:
		100:
			button_texture=get_node(lag_but)
		101:
			button_texture=get_node(normal_but)
		_:
...
func _on_SpecialKeyboardTween_tween_all_completed():
	if anim_stoped:
		used_button_nr=-1
		remove_all()
		button_texture.rect_scale = but_origin_scale
