...
export(NodePath) var no1_but
export(NodePath) var yes1_but
export(NodePath) var no2_but
export(NodePath) var yes2_but
export(NodePath) var no3_but
export(NodePath) var yes3_but
export(NodePath) var no4_but
export(NodePath) var no5_but
export(NodePath) var yes5_but
export(NodePath) var no6_but
export(NodePath) var yes6_but
export(NodePath) var yes7_but
...
func bb_animate_pressing_button(button_nr):
	...
	match button_nr:
		1:
			button_texture=get_node(no1_but)
		2:
			button_texture=get_node(yes1_but)
		3:
			button_texture=get_node(no2_but)
		4:
			button_texture=get_node(yes2_but)
		5:
			button_texture=get_node(no3_but)
		6:
			button_texture=get_node(yes3_but)
		7:
			button_texture=get_node(no4_but)
		8:
			button_texture=get_node(no5_but)
		9:
			button_texture=get_node(yes5_but)
		10:
			button_texture=get_node(no6_but)
		11:
			button_texture=get_node(yes6_but)
		12:
			button_texture=get_node(yes7_but)
...
func _on_WarningsTween_tween_all_completed():
	if anim_stoped:
		remove_all()
		button_texture.rect_scale = but_origin_scale
		button_texture.rect_position = but_origin_position
